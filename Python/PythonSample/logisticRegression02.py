...
# 머신러닝을 위해서는 문자열로는 안되고 수치 데이터로 바꿔줘야 한다.
...
# 결측치에 대하여 나머지 항목들의 평균 값으로 치환한다.
# fillna : 결측치(na)를 채워넣기(fill)
...
# FirstClass 컬럼에는 Pclass의 값이 2일때만 1을, 나머지는 0으로 채워라.
...
# 성별 나이 일등석 이등석 정보를 이용하여 생존 여부 판단하기
...
# fit_transform 함수를 사용하여 이미 한번 fit 했으므로 두 번째 문장에선 transform 함수만 호출하면 된다.
...
# coef_는 w(가중치)값을 의미한다.
# 회귀 분석에서 w의 의미는 y의 데이터에 대한 인과 관계의 정도를 의미한다.
# 성별과 일등석 탑승 여부가 생사를 판단하는 데 가장 영향력을 많이 준다고 볼 수 있다.
print(model.coef_)
print('-' * 30)
