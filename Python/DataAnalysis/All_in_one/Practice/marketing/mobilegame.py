...
실제로는 retention 이외에 함께 고려해야 할 다양한 메트릭들이 있다.
앱내 구매, 게임 플레이 횟수, 친구초대로 인한 referrer 등
본 데이터에서는 retention만 주어져 있기에 한 가지를 주안점을 두어 분석
서비스 운영자, 기획자 차원에서 정말 중요한 메트릭을 정하고 그 것을 기준으로 테스트 결과를 평가하는 것이 중요하다.
"""